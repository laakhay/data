...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "timeframe", nargs="?", default="M1", help="Timeframe enum name, e.g. M1, M5, H1, D1"
    )
    ...
    p.add_argument(
        ...
        default="false",
        choices=["true", "false"],
        help="Emit only closed candles (true/false)",
    )
    p.add_argument(
        "throttle_ms",
        nargs="?",
        type=int,
        default=0,
        help="Throttle open-candle emissions in ms (0 = no throttle)",
    )
    p.add_argument(
        "dedupe",
        nargs="?",
        default="true",
        choices=["true", "false"],
        help="Dedupe same open-candle close price (true/false)",
    )
    return p.parse_args()
...
async def main() -> None:
    ...
    async for bar in ws.stream_ohlcv(
        ...
        dedupe_same_candle=dedupe,
    ):
        print(
            f"{bar.timestamp.isoformat():25} | {bar.symbol:8} | "
            f"{bar.open:>11.2f} | {bar.high:>11.2f} | {bar.low:>11.2f} | {bar.close:>11.2f} | {str(bool(bar.is_closed)):>6}"
        )
