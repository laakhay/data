    ...
    try:
        ...
        async for bar in provider.stream_ohlcv("BTC", Timeframe.M15):
            if shutdown_flag:
                break
            count += 1
            logger.info(f"✅ Received OHLCV bar #{count}: {bar.symbol} - {bar.timestamp} - O:{bar.open} H:{bar.high} L:{bar.low} C:{bar.close} V:{bar.volume}")
            if count >= 3:
    ...
    try:
        ...
        async for trade in provider.stream_trades("BTC"):
            if shutdown_flag:
                break
            count += 1
            logger.info(f"✅ Received trade #{count}: {trade.symbol} - {trade.timestamp} - {trade.price} @ {trade.quantity} ({'BUY' if not trade.is_buyer_maker else 'SELL'})")
            if count >= 5:
    ...
    try:
        ...
        async for ob in provider.stream_order_book("BTC"):
            if shutdown_flag:
                break
            count += 1
            logger.info(f"✅ Received order book update #{count}: {ob.symbol} - {ob.timestamp} - {len(ob.bids)} bids, {len(ob.asks)} asks")
            if ob.best_bid_price and ob.best_ask_price:
                logger.info(f"   Best bid: {ob.best_bid_price}, Best ask: {ob.best_ask_price}, Spread: {ob.spread}")
            if count >= 3:
    ...
    try:
        ...
        async for oi in provider.stream_open_interest("BTC"):
            if shutdown_flag:
                break
            count += 1
            logger.info(f"✅ Received open interest update #{count}: {oi.symbol} - {oi.timestamp} - OI: {oi.open_interest}")
            if count >= 3:
    ...
    try:
        ...
        async for fr in provider.stream_funding_rate("BTC"):
            if shutdown_flag:
                break
            count += 1
            logger.info(f"✅ Received funding rate update #{count}: {fr.symbol} - {fr.timestamp} - Rate: {fr.funding_rate}")
            if count >= 3:
    ...
    try:
        ...
        async for mp in provider.stream_mark_price("BTC"):
            if shutdown_flag:
                break
            count += 1
            logger.info(f"✅ Received mark price update #{count}: {mp.symbol} - {mp.timestamp} - Mark Price: {mp.mark_price}")
            if count >= 3:
    ...
    try:
        ...
        async for liq in provider.stream_liquidations(["BTC"]):
            if shutdown_flag:
                break
            count += 1
            logger.info(f"✅ Received liquidation #{count}: {liq.symbol} - {liq.timestamp} - {liq.side} - {liq.quantity} @ {liq.price}")
            if count >= 3:
        ...
        if count == 0:
            logger.warning("⚠️  No liquidations received (this is normal - liquidations are rare events)")
        return True
    except Exception as e:
...
async def main():
    ...
    results.append(await run_single_test(futures_provider, "Order Book Stream", test_order_book_stream))
    await asyncio.sleep(2)
    
    results.append(await run_single_test(futures_provider, "Open Interest Stream", test_open_interest_stream))
    await asyncio.sleep(2)
    
    results.append(await run_single_test(futures_provider, "Funding Rate Stream", test_funding_rate_stream))
    await asyncio.sleep(2)
    
    results.append(await run_single_test(futures_provider, "Mark Price Stream", test_mark_price_stream))
    await asyncio.sleep(2)
    
    results.append(await run_single_test(futures_provider, "Liquidations Stream", test_liquidations_stream))
    ...
    results.append(await run_single_test(spot_provider, "Order Book Stream (Spot)", test_order_book_stream))
