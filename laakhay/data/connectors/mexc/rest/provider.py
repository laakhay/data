"""MEXC REST connector for direct use by researchers.

This connector provides direct access to MEXC REST endpoints without
...
class MEXCRESTConnector(RESTProvider):
    """MEXC REST connector for direct research use.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize MEXC REST connector.
    ...
    async def fetch_health(self) -> dict[str, object]:
        """Ping MEXC REST API to verify connectivity."""
        path = "/api/v1/contract/ping" if self.market_type == MarketType.FUTURES else "/api/v3/ping"
        start = perf_counter()
    ...
    async def fetch(self, endpoint_id: str, params: dict[str, Any]) -> Any:
        """Fetch data from a MEXC REST endpoint.
