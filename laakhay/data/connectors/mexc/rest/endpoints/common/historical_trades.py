"""MEXC historical trades endpoint definition and adapter.
...
def build_path(params: dict[str, Any]) -> str:
    ...
    # MEXC uses /api/v3/historicalTrades for spot and /api/v1/contract/history/deals for futures
    return (
        "/api/v1/contract/history/deals"
        if market == MarketType.FUTURES
        else "/api/v3/historicalTrades"
    )
...
def build_headers(params: dict[str, Any]) -> dict[str, str]:
    ...
    if not api_key:
        raise ValueError("API key required for MEXC historical trades endpoint")
...
class Adapter(ResponseAdapter):
    """Adapter for parsing MEXC historicalTrades response into Trade list."""

    def parse(self, response: Any, params: dict[str, Any]) -> list[Trade]:
        """Parse MEXC historicalTrades response.
