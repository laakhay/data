"""MEXC order book endpoint definition and adapter.
...
def build_path(params: dict[str, Any]) -> str:
    ...
    # MEXC uses /api/v3/depth for spot and /api/v1/contract/depth for futures
    return "/api/v1/contract/depth" if market == MarketType.FUTURES else "/api/v3/depth"
...
class Adapter(ResponseAdapter):
    """Adapter for parsing MEXC depth response into OrderBook."""

    def parse(self, response: Any, params: dict[str, Any]) -> OrderBook:
        """Parse MEXC depth response.
