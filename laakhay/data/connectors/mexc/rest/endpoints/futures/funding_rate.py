"""MEXC funding rate endpoint definition and adapter.
...
def build_path(params: dict[str, Any]) -> str:
    ...
    if market != MarketType.FUTURES:
        raise ValueError("Funding rate endpoint is Futures-only on MEXC")
    return "/api/v1/contract/funding_rate"
...
class Adapter(ResponseAdapter):
    """Adapter for parsing MEXC fundingRate response into FundingRate list."""

    def parse(self, response: Any, params: dict[str, Any]) -> list[FundingRate]:
        """Parse MEXC fundingRate response.

        Args:
            response: Raw response from MEXC API (list of funding rate data)
        ...
        for row in response or []:
            fr = Decimal(str(row.get("fundingRate", row.get("funding_rate", 0))))
            ts_ms = int(row.get("fundingTime", row.get("funding_time", 0)))
            out.append(
                FundingRate(
                    symbol=symbol,
                    ...
                    funding_rate=fr,
                    mark_price=(
                        Decimal(str(row.get("markPrice", row.get("mark_price"))))
                        if row.get("markPrice") is not None or row.get("mark_price") is not None
                        else None
                    ),
                )
            )
