"""MEXC open interest (historical) endpoint definition and adapter.
...
def build_path(params: dict[str, Any]) -> str:
    ...
    if market != MarketType.FUTURES:
        raise ValueError("Open interest history endpoint is Futures-only on MEXC")
    return "/api/v1/contract/open_interest/history"
...
class Adapter(ResponseAdapter):
    """Adapter for parsing MEXC openInterestHist response into OpenInterest list."""

    def parse(self, response: Any, params: dict[str, Any]) -> list[OpenInterest]:
        """Parse MEXC openInterestHist response.

        Args:
            response: Raw response from MEXC API (list of OI data)
        ...
        for row in response or []:
            ts_ms = row.get("timestamp", row.get("time"))
            oi_str = row.get("sumOpenInterest", row.get("open_interest", row.get("openInterest")))
