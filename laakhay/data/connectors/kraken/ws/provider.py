"""Kraken WebSocket connector for direct use by researchers.

This connector provides direct access to Kraken WebSocket streams without
...
class KrakenWSConnector(WSProvider):
    """Kraken WebSocket connector for direct research use.

    This connector can be used directly without going through DataRouter.
    It provides full access to Kraken WebSocket streams with automatic
    endpoint spec and adapter resolution.
    """

    def __init__(self, *, market_type: MarketType = MarketType.SPOT) -> None:
        """Initialize Kraken WebSocket connector.
    ...
    async def stream_funding_rate(
        ...
        """Stream funding rate updates (Futures-only, via mark price stream).
    ...
    async def stream_liquidations(self) -> AsyncIterator[Liquidation]:
        ...
        Yields:
            Liquidation objects
        """
        async for obj in self.stream("liquidations", ["liquidations"], {}):
            yield obj
    ...
    async def stream(
        ...
        *,
        ...
        """Stream data from a Kraken WebSocket endpoint.
