...
class TimeInterval(str, Enum):
    ...
    Provider implementations must map their native intervals to these values.
    """
    ...
    # Conversion mapping (computed once)
    _SECONDS_MAP: Dict[str, int] = {
        "1m": 60,
        "3m": 180,
        "5m": 300,
        "15m": 900,
    ...
    def seconds(self) -> int:
        ...
        return self._SECONDS_MAP[self.value]
    ...
    def from_seconds(cls, seconds: int) -> "TimeInterval":
        """
        Get interval from seconds value.
        
        Raises:
            ValueError: If no matching interval exists
        """
        for interval in cls:
            if interval.seconds == seconds:
                return interval
        raise ValueError(f"No interval matches {seconds} seconds")
