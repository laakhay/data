    ...
    async def stream(self, topics: list[str]) -> AsyncIterator[Any]:
        ...
        while True:
            try:
                async with websockets.connect(
                    ...
                    for topic in topics:
                        ...
                        if len(parts) >= 2:
                            ...
                            if channel == "candles":
                                logger.warning(f"Skipping candles channel for {product_id} - Exchange API doesn't support WebSocket candles")
                                continue
                            ...
                            if channel == "level2":
                                logger.warning(f"Skipping level2 channel for {product_id} - Exchange API requires authentication for level2")
                                continue
                    ...
                    if all_product_ids and all_channels:
                        subscribe_msg: dict[str, Any] = {
                            "type": "subscribe",
                            "product_ids": sorted(list(all_product_ids)),  # Sort for consistency
                            "channels": sorted(list(all_channels)),  # Exchange API uses "channels" (plural, array)
                        }
                        ...
                        logger.info(f"Sent subscription for {len(all_product_ids)} products ({', '.join(sorted(all_product_ids))}), {len(all_channels)} channels ({', '.join(sorted(all_channels))}) on Coinbase Exchange WebSocket")
                    else:
                        ...
                        try:
                            ...
                            if isinstance(data, dict):
                                ...
                                if msg_type == "subscriptions" or msg_type == "subscribe":
                                    ...
                                    if "error" in str(data).lower() or "reject" in str(data).lower():
                                ...
                                if msg_type == "heartbeat" or msg_type == "pong" or msg_type == "ping":
                                    continue
                ...
                logger.warning(f"Coinbase WebSocket connection closed (code: {e.code}, reason: {e.reason}), reconnecting...")
