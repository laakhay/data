"""OKX REST-only provider.

Implements the RESTProvider interface for OKX API v5.
"""
...
class OKXRESTProvider(RESTProvider):
    """REST-only provider for OKX Spot or Futures."""
    ...
    async def fetch_ohlcv(
        ...
        from ..constants import INTERVAL_MAP as OKX_INTERVAL_MAP
        ...
        if not isinstance(timeframe, Timeframe) or timeframe not in OKX_INTERVAL_MAP:
            raise ValueError(f"Invalid timeframe: {timeframe}")
        ...
        async def _fetch_chunk(
            *,
            ...
            params = {
                ...
                "symbol": symbol,
                "interval": timeframe,
                "interval_str": OKX_INTERVAL_MAP[timeframe],
