"""Unified OKX provider that wraps REST and WebSocket implementations."""
...
class OKXProvider(BaseProvider):
    """High-level OKX provider exposing REST and streaming helpers."""
    ...
    def __init__(
        ...
        *,
        ...
        rest_provider: OKXRESTProvider | None = None,
        ws_provider: OKXWSProvider | None = None,
    ) -> None:
        super().__init__(name="okx")
        self.market_type = market_type
        self._rest = rest_provider or OKXRESTProvider(
            market_type=market_type, api_key=api_key, api_secret=api_secret
        )
        self._ws = ws_provider or OKXWSProvider(market_type=market_type)
