"""Kraken REST-only provider.

Implements the RESTProvider interface for Kraken Spot and Futures APIs.
"""
...
class KrakenRESTProvider(RESTProvider):
    """REST-only provider for Kraken Spot or Futures."""
    ...
    _MAX_CANDLES_PER_REQUEST = 720  # Kraken Spot max is 720, Futures may differ
    ...
    async def fetch_ohlcv(
        ...
        from ..constants import INTERVAL_MAP as KRAKEN_INTERVAL_MAP
        ...
        if not isinstance(timeframe, Timeframe) or timeframe not in KRAKEN_INTERVAL_MAP:
            raise ValueError(f"Invalid timeframe: {timeframe}")
        ...
        async def _fetch_chunk(
            *,
            ...
            params = {
                ...
                "symbol": symbol,
                "interval": timeframe,
                "interval_str": KRAKEN_INTERVAL_MAP[timeframe],
